...
| Gemini CLI | Yes, link the skill folders under `~/.gemini/skills`, `~/.agents/skills`, `.gemini/skills`, or `.agents/skills`; the plugin manifests are ignored | Yes, with shell consent, Node, Playwright, and local profile access |
| Google Antigravity | Yes, link under workspace `.agents/skills` or global `~/.gemini/config/skills`; the plugin manifests are ignored | Yes, when its local agent has shell access and the required dependencies |
...
- Gemini CLI: link the skill folders into `~/.gemini/skills/` or `~/.agents/skills/` for user scope, or `.gemini/skills/` or `.agents/skills/` for workspace scope; within a tier the `.agents/` alias takes precedence. `gemini skills install <path-or-url> --path skills/<name>` also works, but it copies the folder and therefore loses the repository-root `workflow/` runner.
- Antigravity: use workspace `.agents/skills/` or global `~/.gemini/config/skills/`. There is no install command or UI. The global path is the one recognized by the IDE, the CLI, and the agent alike, and it is not the same directory as the Gemini CLI user alias.
- Gemini CLI and Antigravity both ignore `.codex-plugin/plugin.json` and `.claude-plugin/plugin.json`. Link rather than copy the skill folders so that `social-browser-publisher` can still reach `workflow/run.mjs` and `package.json` at the repository root; confirm your installed version follows symlinks before depending on it.
